...
test('Title capitalization according to rule set is correct', () => {
...
test('Lowercase word - lowercaseWord fn', () => {
    const input = 'WOrD';
    const expected = 'word';
    expect(lowercaseword(input)).toEqual(expected);
});

test('Title capitalization according to rule set is correct - capTitle fn', () => {
    const inputTitle = 'the title is in check but not inCorrectly capitalized aNd AT ease';
    const expectedTitle = 'The Title Is in Check but Not Incorrectly Capitalized and at Ease';
    expect(captitle(inputTitle)).toEqual(expectedTitle);
});

test('Capitalize at index position', () => {
    const input = 'WOrds';
    const index = 3;
